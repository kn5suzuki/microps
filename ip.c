...
int ip_endp_pton(const char *p, ip_endp_t *n)
{
    char *sep;
...
struct ip_iface *
ip_iface_alloc(const char *unicatst, const char *netmask)
{
    struct ip_iface *iface;
    ...
    if (ip_addr_pton(unicatst, &iface->unicast) == -1)
    {
        errorf("ip_addr_pton() failure addr=%s", unicatst);
        memory_free(iface);
        return NULL;
    }
...
ssize_t
ip_build_packet(uint8_t protocol, const uint8_t *data, size_t len, uint16_t id, uint16_t offset, ip_addr_t src, ip_addr_t dst, uint8_t *buf, size_t size)
{
    uint16_t hlen, total;
    struct ip_hdr *hdr;
...
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    if (plen == -1)
    {
        errorf("ip_build_packed() failure");
        return -1;
    }
