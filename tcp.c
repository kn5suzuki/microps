...
#define TCP_STATE_CHANGE(x, y)                                                                         \
    do                                                                                                 \
    {                                                                                                  \
        debugf("desc=%d, %s=>%s", tcp_pcb_desc((x)), tcp_state_ntoa((x)->state), tcp_state_ntoa((y))); \
        (x)->state = (y);                                                                              \
    } while (0);
...
tcp_flg_ntoa(uint8_t flg)
{
    ...
    snprintf(str, sizeof(str), "--%c%c%c%c%c%c",
             TCP_FLAG_ISSET(flg, TCP_FLG_URG) ? 'U' : '-',
             TCP_FLAG_ISSET(flg, TCP_FLG_ACK) ? 'A' : '-',
             TCP_FLAG_ISSET(flg, TCP_FLG_PST) ? 'P' : '-',
             TCP_FLAG_ISSET(flg, TCP_FLG_RST) ? 'R' : '-',
             TCP_FLAG_ISSET(flg, TCP_FLG_SYN) ? 'S' : '-',
             TCP_FLAG_ISSET(flg, TCP_FLG_FIN) ? 'F' : '-');
    return str;
}
...
tcp_pcb_release(struct tcp_pcb *pcb)
{
    ...
    while (1)
    {
        ...
        debugf("release backlog entry, desc=%d, state=%s", tcp_pcb_desc(backlog), tcp_state_ntoa(backlog->state));
...
tcp_pcb_select(ip_endp_t key1, ip_endp_t key2)
{
    ...
    for (pcb = pcbs; pcb < tailof(pcbs); pcb++)
    {
        ...
        if (pcb->local.addr == key1.addr || pcb->local.addr == IP_ADDR_ANY || key1.addr == IP_ADDR_ANY)
        {
            if ((pcb->remote.addr == key2.addr && pcb->remote.port == key2.port) || (pcb->remote.addr == IP_ADDR_ANY && pcb->remote.port == 0) || (key2.addr == IP_ADDR_ANY && key2.port == 0))
            {
...
static ssize_t
tcp_output_segment(uint32_t seq, uint32_t ack, uint8_t flg, uint16_t wnd, const uint8_t *data, size_t len, ip_endp_t local, ip_endp_t remote)
{
    ...
    debugf("%s => %s, len=%zu", ip_endp_ntop(local, ep1, sizeof(ep1)), ip_endp_ntop(remote, ep2, sizeof(ep2)), total);
    tcp_print(buf, total);
...
tcp_output(struct tcp_pcb *pcb, uint8_t flg, const uint8_t *data, size_t len)
{
    ...
    seq = pcb->snd.nxt;
    if (TCP_FLAG_ISSET(flg, TCP_FLG_SYN))
    {
        seq = pcb->iss;
    }
...
tcp_segment_arrives(struct seg_info *seg, uint8_t flags, const uint8_t *data, size_t len, ip_endp_t local, ip_endp_t remote)
{
    ...
    if (!pcb || pcb->state == TCP_STATE_CLOSED)
    {
        ...
        if (TCP_FLAG_ISSET(flags, TCP_FLG_RST))
        {
            return;
        }
        if (!TCP_FLAG_ISSET(flags, TCP_FLG_ACK))
        {
            tcp_output_segment(0, seg->seq + seg->len, TCP_FLG_RST | TCP_FLG_ACK, 0, NULL, 0, local, remote);
        }
        else
        {
    ...
    case TCP_STATE_LISTEN:
        ...
        if (TCP_FLAG_ISSET(flags, TCP_FLG_SYN))
        {
            if (pcb->mode == TCP_PCB_MODE_SOCKET)
            {
                ...
                debugf("allocate PCB for new connection, desc=%d, state=%s", tcp_pcb_desc(new_pcb), tcp_state_ntoa(new_pcb->state));
                new_pcb->parent = pcb;
                pcb = new_pcb;
            }
            pcb->local = local;
    ...
    case TCP_STATE_SYN_SENT:
        ...
        if (TCP_FLAG_ISSET(flags, TCP_FLG_ACK))
        {
            ...
            {
                tcp_output_segment(seg->seq, 0, TCP_FLG_RST, 0, NULL, 0, local, remote);
                return;
            }
...
static void tcp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface)
{
    ...
    ip_endp_ntop(src, ep2, sizeof(ep2));
    if (src.addr == IP_ADDR_BROADCAST || src.addr == iface->broadcast || dst.addr == IP_ADDR_BROADCAST || dst.addr == iface->broadcast)
    {
    ...
    seg.len = len - hlen;
    if (TCP_FLAG_ISSET(hdr->flg, TCP_FLG_SYN))
    {
        seg.len++;
    }
    seg.wnd = ntoh16(hdr->seq);
...
int tcp_cmd_open(ip_endp_t local, ip_endp_t remote, int active)
{
    ...
    debugf("mode=%s, local=%s, remote=%s", active ? "active" : "passive", ip_endp_ntop(local, ep1, sizeof(ep1)), ip_endp_ntop(remote, ep2, sizeof(ep2)));
    if (active)
    {
        if (local.addr == IP_ADDR_ANY)
        {
            ...
            if (!iface)
            {
                errorf("iface not found that can reach remote address, addr=%s", ip_addr_ntop(remote.addr, addr, sizeof(addr)));
                lock_release(&lock);
                return -1;
            }
        ...
        if (!local.port)
        {
            for (p = IP_ENDP_DYNAMIC_PORT_MIN; p <= IP_ENDP_DYNAMIC_PORT_MAX; p++)
            {
                ...
                {
                    debugf("dynamic assign local port, port=%d", ntoh16(local.port));
                    break;
                }
            }
...
AGAIN:
    ...
    debugf("success, local=%s, remote=%s", ip_endp_ntop(local, ep1, sizeof(ep1)), ip_endp_ntop(remote, ep2, sizeof(ep2)));
    lock_release(&lock);
    return desc;
}
...
int tcp_cmd_connect(int desc, ip_endp_t remote)
{
    ...
    local = pcb->local;
    debugf("local=%s, remote=%s", ip_endp_ntop(local, ep1, sizeof(ep1)), ip_endp_ntop(remote, ep2, sizeof(ep2)));
    if (local.addr == IP_ADDR_ANY)
    {
        iface = ip_route_get_iface(remote.addr);
        if (!iface)
...
AGAIN:
    ...
    pcb->mss = NET_IFACE(iface)->dev->mtu - (IP_HDR_SIZE_MIN + sizeof(struct tcp_hdr));
    debugf("success, local=%s, remote=%s", ip_endp_ntop(pcb->local, ep1, sizeof(ep1)), ip_endp_ntop(pcb->remote, ep2, sizeof(ep2)));
    lock_release(&lock);
    return 0;
}
...
int tcp_cmd_accept(int desc, ip_endp_t *remote)
{
    ...
    debugf("succes: desc=%d, local=%s, remote=%s", new_desc, ip_addr_ntop(new_pcb->local.addr, ep1, sizeof(ep1)), ip_addr_ntop(new_pcb->remote.addr, ep2, sizeof(ep2)));
    lock_release(&lock);
    return new_desc;
}
...
RETRY:
    ...
    case TCP_STATE_CLOSE_WAIT:
        while (sent < (ssize_t)len)
        {
            ...
            if (tcp_output(pcb, TCP_FLG_ACK | TCP_FLG_PST, data + sent, slen) == -1)
            {
                errorf("tcp_output() failure");
                TCP_STATE_CHANGE(pcb, TCP_STATE_CLOSED);
                tcp_pcb_release(pcb);
