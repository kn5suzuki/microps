...
int sock_open(int domain, int type, int protocol)
{
    ...
    case SOCK_STREAM:
        ...
        break;
    case SOCK_DGRAM:
        if (protocol != 0 && protocol != IP_PROTOCOL_UDP)
        {
            return -1;
        }
        break;
...
ssize_t
sock_sendto(int desc, const void *buf, size_t n, const struct sockaddr *addr)
{
    struct sock *s, sock;
    ip_endp_t remote;
...
int sock_bind(int desc, const struct sockaddr *addr)
{
    struct sock *s, sock;
    ip_endp_t local;
...
int sock_accept(int desc, const struct sockaddr *addr, int *addrlen)
{
...
int sock_connect(int desc, const struct sockaddr *addr)
{
    struct sock *s, sock;
    ip_endp_t remote;
...
sock_recv(int desc, void *buf, size_t n)
{
    ...
    case AF_INET:
        ...
        {
        case SOCK_STREAM:
            return tcp_cmd_receive(sock.desc, buf, n);
        default:
            errorf("unsupported type %d", sock.type);
            return -1;
        }
