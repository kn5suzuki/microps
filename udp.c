...
struct udp_queue_entry
{
    ...
    ip_endp_t remote;
    uint16_t len;
    /* data byutes exists after this structure. */
};
...
udp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface)
{
    ...
    dst.addr = iphdr->dst;
    dst.port = hdr->dst;
    debugf("%s => %s, len=%zu, dev=%s", ip_endp_ntop(src, endp1, sizeof(endp1)), ip_endp_ntop(dst, endp2, sizeof(endp2)), len, NET_IFACE(iface)->dev->name);
    udp_print(data, len);
    lock_acquire(&lock);
...
udp_output(ip_endp_t src, ip_endp_t dst, const uint8_t *data, size_t len)
{
    ...
    debugf("%s => %s, len=%zu", ip_endp_ntop(src, endp1, sizeof(endp1)), ip_endp_ntop(src, endp2, sizeof(endp2)), total);
    udp_print(buf, total);
...
int udp_cmd_bind(int desc, ip_endp_t local)
{
    ...
    if (exist)
    {
        errorf("already in use, desc=%d, want=%s, exist=%s", desc, ip_endp_ntop(local, endp1, sizeof(endp1)), ip_endp_ntop(local, endp2, sizeof(endp2)));
        lock_release(&lock);
        return -1;
    }
    pcb->local = local;
    debugf("desc=%s, %d", desc, ip_endp_ntop(local, endp1, sizeof(endp1)));
    lock_release(&lock);
    return 0;
}
...
udp_cmd_sendto(int desc, uint8_t *data, size_t len, ip_endp_t remote)
{
    ...
    if (!local.port)
    {
        for (p = IP_ENDP_DYNAMIC_PORT_MIN; p <= IP_ENDP_DYNAMIC_PORT_MAX; p++)
        {
            ...
            if (!udp_pcb_select(local))
            {
                ...
                debugf("dynamic assign local port, port=%d", ntoh16(pcb->local.port));
                break;
            }
        }
    ...
    if (local.addr == IP_ADDR_ANY)
    {
        ...
        if (!iface)
        {
            errorf("iface not found that can reach foreign address, address=%s", ip_addr_ntop(remote.addr, addr, sizeof(addr)));
            lock_release(&lock);
            return -1;
        }
