...
app_main(void)
{
    ...
    if (sock_bind(soc, (struct sockaddr *)&local) == -1)
    {
    ...
    while (!terminate)
    {
        ...
        debugf("connection accepted, remote=%s:%u", ip_addr_ntop(remote.sin_addr.s_addr, addr, sizeof(addr)), ntoh16(remote.sin_port));
        conn_main(acc);
    }
    sock_close(soc);
    debugf("terminate");
