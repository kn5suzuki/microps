...
receiver(void *arg)
{
    ...
    while (!terminate)
    {
        ...
        infof("%d bytes data receive from %s", n, ip_endp_ntop(remote, endp, sizeof(endp)));
...
app_main(void)
{
    ...
    while (!terminate)
    {
        ...
        n = strlen((char *)buf);
        infof("%d bytes data send to %s", n, ip_endp_ntop(remote, endp, sizeof(endp)));
        hexdump(stderr, buf, n);
        if (udp_cmd_sendto(desc, buf, n, remote) == -1)
        {
            errorf("udp_cmd_sentdo() failure");
            break;
        }
    }
    udp_cmd_close(desc);
